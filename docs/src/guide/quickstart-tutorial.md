...
This tutorial shows the full AgentForge in Agent Platform experience — you talk to your coding agent, and it builds, evaluates, and deploys an ADK agent for you.
...
> *"Use agentforge to build a caveman-style agent that compresses verbose text into terse, technical grunts"*

Your coding agent activates the `agentforge-workflow` and `agentforge-scaffold` skills. It will:
...
```
agentforge create caveman-agent --prototype --yes
cd caveman-agent && agentforge install
```
...
Your coding agent edits `app/agent.py` — replacing the default agent with your caveman compressor. It uses the `agentforge-adk-code` skill for ADK patterns.
...
```
agentforge run "Please help me understand the deployment options available for my project"
```
...
Your coding agent activates the `agentforge-eval` skill and:
...
Your coding agent activates the `agentforge-deploy` skill and:

- Adds deployment infrastructure:

```
agentforge scaffold enhance --deployment-target cloud_run
```
