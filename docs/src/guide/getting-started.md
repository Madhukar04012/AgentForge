...
**AgentForge** is a CLI and skills package for building, evaluating, and deploying AI agents on Google Cloud. Agents are built with Google's [Agent Development Kit (ADK)](https://google.github.io/adk-docs/) — AgentForge handles everything around it: scaffolding, evaluation, deployment, and observability.
...
AgentForge bundles **7 skills** that give your coding agent deep knowledge across the full ADK lifecycle:

| Skill | What your coding agent learns |
|-------|-------------------------------|
| `agentforge-workflow` | Development lifecycle, code preservation, model selection |
| `agentforge-adk-code` | ADK Python API — agents, tools, orchestration, callbacks |
| `agentforge-scaffold` | Project scaffolding — `create`, `enhance`, `upgrade` |
| `agentforge-eval` | Evaluation lifecycle — datasets, metrics, generate/grade, compare, analyze, optimize |
| `agentforge-deploy` | Deployment — Agent Runtime, Cloud Run, GKE, CI/CD |
| `agentforge-publish` | Gemini Enterprise registration |
| `agentforge-observability` | Cloud Trace, logging, third-party integrations |
...
??? info "Alternative installation methods"
    **pipx:** `pipx install agentforge && agentforge setup`

    **venv + pip:** `pip install agentforge && agentforge setup`

    **Skills only:** `npx skills add google/agentforge`
...
If you're already authenticated with `gcloud`, it just works — AgentForge picks up your Application Default Credentials automatically.
...
=== "Antigravity CLI"
    ...
    2. **Verify skills are installed**
        ...
        Check that AgentForge skills are available in your environment.
...
=== "Claude Code"
    ...
    2. **Verify skills are installed**
        ...
        You should see `agentforge-workflow` and other AgentForge skills listed.
...
=== "Any Other Agent"
    ...
    AgentForge works with any coding agent that supports [skills](https://agentskills.io/what-are-skills).
    ...
    2. **Verify skills are visible**
        ...
        Check that your agent can see `agentforge-workflow` and other AgentForge skills. Most agents expose this via a `/skills` command or settings panel.
...
agentforge create my-agent --prototype --yes
...
!!! note "Share what you build"
    Built something interesting with AgentForge? We'd love to hear about it! Share your project at [agents-cli@google.com](mailto:agents-cli@google.com).
