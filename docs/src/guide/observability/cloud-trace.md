...
Cloud Trace is enabled by default in all `agentforge` projects. This guide shows how to verify it works and query your telemetry data.
...
Prompt-response logging to GCS and BigQuery is provisioned by Terraform (`agentforge infra single-project` or `agentforge infra cicd`), which creates the logs bucket and dataset and sets `LOGS_BUCKET_NAME` — it's enabled automatically there. A bare `agentforge deploy` does **not** create these resources, so the checks below apply only to Terraform-provisioned deployments.
...
By default, `agentforge playground` runs **without** prompt-response logging. Telemetry is
