...
AgentForge is opinionated about one thing: the loop between **"looks good in a notebook"** and **"live in production."** This page is the map.
...
| | Without the loop | With AgentForge |
|---|---|---|
...
|---|---|---|---|---|---|
| 0 | **Spec** | Write a `.agents-cli-spec.md`. The other phases derive from this. | — | `agentforge-workflow` | [Development Guide](development.md) |
| 1 | **Scaffold** | Turn the spec into a production-shaped project (~72 files). | `scaffold create` | `agentforge-scaffold` | [Development Guide](development.md#phase-1-scaffold) |
| 2 | **Build** | Write the agent body — model, instruction, tools, `App` wrapper. | — | `agentforge-adk-code` | [Project Structure](project-structure.md) |
| 3 | **Orchestrate** | Compose specialists when one agent grows into a team. | — | `agentforge-adk-code` | [Project Structure](project-structure.md) |
| 4 | **Evaluate** | Score the agent against a dataset before every deploy. | `eval run` (or `eval generate` + `eval grade`), plus `eval dataset synthesize`, `eval compare`, `eval analyze`, `eval metric list`, and `eval optimize` | `agentforge-eval` | [Evaluation](evaluation.md) |
| 5 | **Deploy** | Ship to Agent Runtime, Cloud Run, or GKE. | `deploy` | `agentforge-deploy` | [Deployment](deployment.md) |
| 6 | **Publish** | Register with Gemini Enterprise so other agents can find this one. | `publish` | `agentforge-publish` | [CI/CD](cicd.md) |
| 7 | **Observe** | Cloud Trace + BigQuery analytics; production data feeds tomorrow's dataset. | — | `agentforge-observability` | [Observability](observability/index.md) |
...
When the team needs to span processes — or call agents your team doesn't own — use the **[A2A protocol](https://a2a-protocol.org/)** as the wire format. A2A is built into every ADK agent, so just scaffold normally (`--agent adk`): any A2A-compatible agent (built with AgentForge or not) can call yours, and yours can call theirs.

### 4 · Evaluate

This is the phase most agent demos skip. `agentforge eval run` can execute your dataset against the live agent, ask an LLM judge to score each response against a rubric, and give you a number you can defend.
...
Expect 5–10+ iterations of the `agentforge eval run` loop. Every fix nudges the score, you re-run, you ship when it crosses the threshold. Below: the four failure modes the rubrics catch most often.
...
The same agent code can land in three different places. `agentforge deploy` dispatches based on the target you scaffolded with. **Pick one to see what `--dry-run` would print and the steps that would follow:**
...
```bash
agentforge deploy --dry-run        # preview the pipeline
agentforge deploy                  # ship it
agentforge deploy --no-wait        # return immediately; check later with --status
```

Each target inherits the surrounding production primitives:

- **Per-agent service account** — opt in with `agentforge deploy --agent-identity`, and the deployed agent runs as its own GCP identity. Scope what it can actually call (which BigQuery datasets, which buckets, which APIs) with normal IAM. The eval rubrics that block destructive remediations have a fallback: the agent literally can't `kubectl delete` if its identity isn't allowed to.
...
=== "Ask your coding agent"
    ...
    2. Run `agentforge scaffold create … --agent adk --deployment-target agent_runtime` (RAG is a clone-and-study recipe — adapt a RAG sample; see [Extending the starter agent](development.md#extending-the-starter-agent))
    3. Author the agent body and tools
    4. Write dataset cases
    5. Run `agentforge eval run` and iterate until the score crosses threshold
    6. Run `agentforge deploy`
...
=== "Drive the CLI yourself"
    ...
    ```bash
    # Phase 1: scaffold
    agentforge scaffold create outage-recovery-bot \
      --agent adk \
      --deployment-target agent_runtime \
      --cicd-runner github_actions \
      --bq-analytics
    cd outage-recovery-bot && agentforge install

    # Phase 2-3: build & orchestrate (edit app/agent.py)
    agentforge playground       # local web playground at :8080

    # Phase 4: evaluate
    agentforge eval dataset synthesize --count 10  # optional: cold-start a dataset
    agentforge eval run                            # repeat until eval score crosses threshold
    agentforge eval compare prev.json latest.json  # confirm fixes actually helped
    agentforge eval analyze --eval-result latest.json  # cluster remaining failures
    agentforge eval optimize                       # optional: auto-tune prompts using eval data
    ...
    agentforge publish gemini-enterprise
    ```
