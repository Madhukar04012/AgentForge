...
agentforge sits on top of multiple tools, each with its own authentication needs. This page breaks down the three distinct levels so you understand exactly what you're authenticating and why.

---

## Level 1: Coding Agent Auth

Your coding agent (Antigravity CLI, Claude Code, Codex, etc.) needs its own authentication to function. **agentforge does not control this** — each agent handles its own credentials.
...
Refer to your coding agent's documentation for setup instructions. This is independent of AgentForge.
...
3. Save the file — the `.env` is loaded automatically when you run `agentforge` dev commands.
...
- `agentforge deploy` — deploy to Agent Runtime, Cloud Run, or GKE
- `agentforge infra single-project` / `agentforge infra cicd` — provision infrastructure and CI/CD with Terraform
