...
```bash
agentforge create my-first-agent --prototype --yes
cd my-first-agent
agentforge install
```
...
- `agentforge install` installs all Python dependencies via `uv sync`.
...
```bash
agentforge run "What's the weather in San Francisco?"
```
...
```bash
agentforge run "How many words are in: The quick brown fox jumps over the lazy dog"
```
...
```bash
agentforge infra single-project --project YOUR_DEV_PROJECT_ID
```
...
| Step | What happened |
|------|--------------|
| `agentforge create --prototype --yes` | Created a project with agent code, tests, and eval sets |
| `agentforge playground` | Started the ADK playground for interactive testing |
| `agentforge run "..."` | Tested the agent from the terminal |
| Edited `agent.py` | Customized the persona and added a tool |
| `agentforge eval run` | Validated agent behavior with structured evaluations |
| `agentforge deploy` | Deployed the agent to Google Cloud |
