...
!!! note "Upgrading from an older agentforge?"
...
agentforge eval run --dataset tests/eval/datasets/custom-dataset.json --metrics general_quality

# Evaluate a deployed agent instead of running one locally
agentforge eval run --url https://my-agent.run.app --app-name app
```
...
```bash
agentforge eval generate --dataset tests/eval/datasets/custom-dataset.json --output custom_traces/
agentforge eval grade --metrics general_quality --traces custom_traces/
```
...
```bash
agentforge eval metric list
```

#### Common metrics at a glance

A short reference for the most-used built-in metric IDs. Use `agentforge eval metric list` for the full set with descriptions.
...
```bash
agentforge eval dataset synthesize --count 10
```
...
```bash
agentforge eval analyze --eval-result grade_results.json
```
