...
A framework is delivered as a **template you scaffold from**, not as an extension you install. The template carries its own `agentforge-extension.yaml`, so the project gets the framework's command overrides with nothing installed machine-wide.

| Framework | What it does |
|--------|--------------|
| [LangChain](#langchain) | Run a LangChain agent through the `agentforge` lifecycle. |

---

## LangChain

The LangChain template runs a [LangChain](https://python.langchain.com) agent through the `agentforge` lifecycle. It overrides only the framework-coupled commands and leaves everything else — `deploy`, `infra`, `eval grade` — running natively.
...
agentforge create my-agent \
    ...
    -d cloud_run
cd my-agent && agentforge install
```

The scaffolded project contains `agentforge-extension.yaml` at its root. That file is auto-loaded at project scope, so the overrides below are active in this project and nowhere else. Commit it.
...
Query a deployed (or locally served) agent over A2A. Because the project overrides `run` with in-process graph invocation, set `AGENTFORGE_DISABLE_OVERRIDES=1` to reach the built-in A2A client:

```bash
AGENTFORGE_DISABLE_OVERRIDES=1 agentforge run --url https://<service-url> --mode a2a --app-name app "hello"
```

### Full journey

```bash
agentforge create my-agent --agent google/agents-cli/extensions/langchain/template@v1.5.0 -d cloud_run
cd my-agent && agentforge install
agentforge run "what's the weather in San Francisco?"       # in-process graph
...
agentforge deploy                                            # native Cloud Run deploy
```
...
- **Run the built-in instead of an override:** prefix with `AGENTFORGE_DISABLE_OVERRIDES=1` (e.g. `AGENTFORGE_DISABLE_OVERRIDES=1 agentforge run "hi"`).
