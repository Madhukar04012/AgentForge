...
When you run `agentforge create my-agent --prototype --yes`, you get a ready-to-run project. This page explains what each file does.
...
├── agentforge-manifest.yaml      # Configuration for agentforge
...
Contains agentforge project metadata and configuration:

```yaml title="agentforge-manifest.yaml"
name: my-agent
agent_directory: app
create_params:
...
- **`agent_directory`** — tells `agentforge` commands where your agent code is.
- **`create_params`** — records how the project was created. Used by `agentforge scaffold upgrade` to preserve your configuration.
...
When you create a project with a deployment target (or add one with `agentforge scaffold enhance`), additional directories appear:
