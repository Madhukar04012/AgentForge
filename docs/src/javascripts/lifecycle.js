...
(function () {
  ...
  function initTranscript(host) {
    ...
    head.appendChild(htmlEl("span", { class: "lc-transcript__title" }, "$ agentforge playground · root_agent"));
  ...
  function initEvalLoop(host) {
    ...
    host.appendChild(htmlEl("div", { class: "lc-eval__head" }, [
      htmlEl("span", { class: "lc-mono" }, "$ agentforge eval run"),
  ...
  const DEPLOY_TARGETS = [
    ...
    },
    {
      ...
      cmd: "agentforge deploy --dry-run\n  Would run: gcloud beta run deploy outage-recovery-bot --project outage-recovery-prod --region us-east1 --source . --memory 4Gi --no-allow-unauthenticated --no-cpu-throttling --port 8080 --labels created-by=agentforge",
      pipeline: [
  ...
  function initDeploy(host) {
    ...
    function render() {
      ...
      pipelineHead.textContent = "what `agentforge deploy` runs · target = " + t.id;
  ...
  function initPublish(host) {
    ...
    const cmd = htmlEl("pre", { class: "lc-publish__cmd" }, htmlEl("code", null,
      "$ agentforge publish gemini-enterprise --registration-type adk\n✅ Successfully registered agent to Gemini Enterprise!"));
