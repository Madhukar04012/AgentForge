...
**Steps:**
1. If prototype, first add Terraform/CI-CD files using the AgentForge (see `/agentforge-scaffold` for full options):
   ```bash
...
Run `agentforge infra cicd --help` for the full flag reference (Cloud Build options, dev project, region, etc.).
...
The `agentforge infra cicd` command only supports GitHub. It requires the `gh` CLI, uses the Terraform `github` provider, and both CI/CD runners (GitHub Actions, Cloud Build) assume a GitHub source repo.

For other git providers, use the scaffolded Terraform as a starting point:
1. Run `agentforge scaffold enhance` to generate the Terraform and CI/CD files
...
4. Run `terraform apply` directly instead of `agentforge infra cicd`
