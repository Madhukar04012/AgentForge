...
> **Assumes `/agentforge-scaffold` scaffolding.** If your project isn't scaffolded yet, see `/agentforge-scaffold` first.

## Deployment Architecture

Agent Runtime uses **container-based deployment**: `agentforge deploy` packages your project and Agent Engine builds a container image from your project's `Dockerfile` (required at the project root; scaffolded projects ship one).
...
`agentforge deploy` always labels the deployment `agent_framework = "google-adk"`
...
Deploy with `agentforge deploy` (run `agentforge deploy --help` for the full flag reference). CI/CD pipelines invoke the same command.

**Deployment flow:**
1. `agentforge deploy` packages the project files (honoring `.gcloudignore`/`.gitignore`)
...
Key difference from Cloud Run: the `lifecycle.ignore_changes` (covering `container_spec`, `source_code_spec`, and `deployment_spec`) is critical — the image and source are updated by `agentforge deploy` / CI/CD, not Terraform.

## deployment_metadata.json

Written by `agentforge deploy` after a successful deployment:
...
Used by: subsequent deploys (update vs create), `agentforge run --url`, and `agentforge publish` (reads the runtime ID for the default ADK registration on Agent Runtime, and constructs the A2A card URL only when A2A registration is explicitly chosen). Cloud Run does not use this file.
...
| **Deploy command** | `agentforge deploy` | `gcloud run deploy --image ...` |
...
agentforge run --url https://LOCATION-aiplatform.googleapis.com/v1/projects/PROJECT/locations/LOCATION/reasoningEngines/ID --mode adk "Hello, what can you do?"
```
...
Environment variables set during deployment come from `agentforge deploy` (the CLI's `deploy/agent_runtime.py`) for SDK deploys, and from `deployment/terraform/single-project/service.tf` (or the `cicd/` variant) for Terraform-managed deploys. Check those for current values.
