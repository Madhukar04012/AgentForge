...
> Requires: agentforge ~= 1.5.0
> If version is behind, run: uv tool install "agentforge~=1.5.0"

> Check version: agentforge info
...
Re-read the relevant skill **before** each phase — not after you've already started and hit a problem. Context compaction may have dropped earlier skill content. If skills are not available, run `uvx agentforge setup` to install them.

| Phase | Skill | When to load |
|-------|-------|--------------|
| 0 — Understand | — | No skill needed — read `.agentforge-spec.md` if present, else clarify goals with the user |
| 1 — Study recipes | `/agentforge-adk-code` | **Load it during design** — its `references/samples.md` topic index maps a need to the recipe that implements it. Yes, this early: the catalog lives there. |
| 2 — Scaffold | `/agentforge-scaffold` | Before creating or enhancing a project |
| 3 — Build | `/agentforge-adk-code` | Before writing agent code — API patterns, tools, callbacks, state |
| 4 — Evaluate | `/agentforge-eval` | Before running any eval — dataset schema, metrics, eval-fix loop |
| 5 — Deploy | `/agentforge-deploy` | Before deploying — target selection, troubleshooting 403/timeouts |
| 6 — Publish | `/agentforge-publish` | After deploying, if registering with Gemini Enterprise (optional) |
| 7 — Observe | `/agentforge-observability` | After deploying — traces, logging, monitoring setup |

---

## Setup

If `agentforge` is not installed:
```bash
...
If `.agentforge-spec.md` exists in the current directory, read it — it is your primary source of truth. Otherwise:
...
Once the design is agreed, write the spec to `.agentforge-spec.md` using the template in `references/spec-template.md`, self-review it, then get the user's approval. See `/agentforge-scaffold` for how these choices map to CLI flags.
...
First check whether a project already exists: run `agentforge info` from the project root. If one was already created or enhanced by agentforge, skip this phase.

Otherwise, scaffold **before writing any code**:
- **No project yet** → `agentforge scaffold create <name>`
- **Existing code to import** → `agentforge scaffold enhance .` (adds the agentforge structure)

Use `/agentforge-scaffold` for the full workflow — it covers architecture choices (deployment target, agent type, session storage) and project creation or enhancement.
...
2. **Quick smoke test**: Use `agentforge run "your prompt"` to verify the agent works after changes — this is the fastest way to check behavior without leaving the terminal
3. Iterate on the implementation based on user feedback

If the user asks for interactive testing, suggest `agentforge playground` — it opens a web-based playground for manual conversation with the agent.

For ADK API patterns and code examples, use `/agentforge-adk-code`.

> **Smoke-test only here — do not write behavioral pytest.** LLM output is non-deterministic; behavioral checks belong in eval (Phase 4), not pytest. Use `agentforge run "prompt"` for quick checks.
...
`agentforge` has no command for this.

## Phase 4: Evaluate

**This is the most important phase.** Evaluation validates agent behavior end-to-end.

**MANDATORY:** Activate `/agentforge-eval` before running evaluation.
...
**`uv run pytest` vs `agentforge eval` — know the difference:**
...
- **`agentforge eval`** — Tests *agent behavior*: response quality, tool usage, persona consistency, safety compliance. This is what validates your agent actually works.
- **`agentforge run "prompt"`** — Quick one-off smoke test during development. If testing multiple prompts use the `--start-server` option to persist the local server, which reduces overhead for repeated calls and allows resuming local sessions via `--session-id`. Use this for fast iteration, not pytest.
...
2. Run evaluations: `agentforge eval run` (chains `generate` + `grade`). For debugging or custom trace locations, use the two-step form: `agentforge eval generate` then `agentforge eval grade`.
...
1. Check if the project has a deployment target configured — run `agentforge info` to see current config
2. If the project is a prototype (no deployment target), add deployment support first:
   ```bash
   agentforge scaffold enhance . --deployment-target <target>
   ```
   See `/agentforge-deploy` for the deployment target decision matrix (Agent Runtime vs Cloud Run vs GKE).
3. Deploy when ready: `agentforge deploy`

**IMPORTANT**: Never deploy without explicit human approval.

## Phase 6: Publish (optional)

Not all agents require this — currently supporting Gemini Enterprise. See `/agentforge-publish` for registration modes, flags, and troubleshooting.

## Phase 7: Observe

After deploying, use observability tools to monitor agent behavior in production. See `/agentforge-observability` for Cloud Trace, prompt-response logging, BigQuery Analytics, and third-party integrations.
...
| "The agent responded correctly in `agentforge run`, so eval isn't needed" | One prompt is not a test suite. Eval catches regressions, edge cases, and tool trajectory issues that a single run never will. |
...
| "I already know how to build this — sandbox, memory, approval gates" | You are about to reinvent a recipe, worse. These capabilities are already implemented and battle-tested in `/agentforge-adk-code` → `references/samples.md`; check the topic index and open the matching recipe's `AGENTS.md` before designing anything custom. |
| "I can skip the scaffold and set up manually" | Manual setup misses eval boilerplate, CI/CD config, and project configuration manifest conventions. Use `agentforge create` even for quick experiments. |
...
- **Troubleshooting:**
  - Check `/agentforge-adk-code` first — it covers most common patterns
  ...
  - **CLI command failures:** run `agentforge <command> --help` — the output ends with a `Source:` line pointing to the exact source file implementing that command. Read it to understand the logic and diagnose failures. Use `agentforge info` to get the full CLI install path if you need to browse across multiple files.
...
2. **Localize** — Narrow the cause: is it the agent code, a tool, the config, or the environment? Use `agentforge run "prompt"` to isolate agent behavior from deployment issues. Add `-v` (`--verbose`) to print the full JSON event payloads — useful for inspecting tool calls, intermediate steps, and silent failures.
...
- **Environment Variables:**
  ...
  - For secrets and API keys, prefer GCP Secret Manager over plain `.env` entries — see `/agentforge-deploy` for secret management guidance

---

## Using a Temporary Scaffold as Reference

When you need specific infrastructure files (Terraform, CI/CD, Dockerfile) but don't want to modify the current project, use `/agentforge-scaffold` to create a temporary project in `/tmp/` and copy over what you need.
...
| File | Contents |
|------|----------|
| `references/internals.md` | Underlying tools and commands that `agentforge` wraps (adk, pytest, ruff, uvicorn) |
| `references/spec-template.md` | `.agentforge-spec.md` template and optional sections |
...
| `references/commands.md` | Per-phase `agentforge` command index |
| `references/extension.md` | Author an ad-hoc extension or adopt an existing one (override or add commands) |

## Development Commands

Run `agentforge --help` or `agentforge <command> --help` for the authoritative flag list. A per-phase command index lives in `references/commands.md`; per-phase usage is in the phase sections above.

To **override** a built-in command (e.g. wrap `deploy` with compliance checks) or **add a new one**, agentforge has an extension system. To author an ad-hoc extension for this repo, or adopt an existing one, load `references/extension.md`.
