...
Turn the user's idea into an agreed `.agentforge-spec.md` through a collaborative dialogue —
*before* any sample study, scaffolding, or code. Adapt the depth to the agent's complexity.

## HARD-GATE

Do NOT clone a sample, scaffold, run `agentforge create`, or write any code until the user has
...
ship to its recipe (`/agentforge-adk-code` → `references/samples.md`) — then commit to a design. The user-review gate is how they catch a wrong assumption.
...
- **Capabilities the scaffold doesn't ship** — retrieval ("search / look up over our docs, incidents,
  ...
  `/agentforge-adk-code` → `references/samples.md` and list every matching recipe in the spec's
...
Once the design is agreed, write `.agentforge-spec.md` using `references/spec-template.md`. Write it to
the **project's working directory** (the cwd where the user is building), not a temporary/scratch
location — Phase 0 resumes by reading `./.agentforge-spec.md`, so a spec saved elsewhere is lost next
...
Ask the user to review `.agentforge-spec.md` before moving on:

> "Spec written to `.agentforge-spec.md`. Please review it and tell me if you want changes before we
