...
An extension overrides or adds a built-in command. It's a directory containing an `agentforge-extension.yaml`, so any git repo can serve as a registry.
...
Drop a single file, `agentforge-extension.yaml`, at the project root (next to `agentforge-manifest.yaml`). It's auto-loaded at project scope, so no `extension add` is needed. Commit it and teammates and CI get the same overrides.

Only one, at that exact path, and always project scope. For a second one, or to install a local extension globally, move it into its own directory and `agentforge extension add local@./path --global`; `#name` picks one out of a directory holding several.

**Publish it for other repos:** move the same file (and its scripts) into its own git repo and tag it — others then `agentforge extension add <org>/<repo>#<name> --ref v1.0.0`. The file doesn't change; ad-hoc and shared are the same format.

### Schema by example (`agentforge-extension/v1alpha1`)
...
requires:
  agents_cli: ">=1.3,<2"   # example - derive from `agentforge --version`, see below
...
- **`run:` is a command vector** executed with **no shell**; user argv is appended verbatim. Paths relative to the extension dir resolve to absolute, and `$AGENTFORGE_EXTENSION_DIR` locates sibling scripts/templates.
...
- **Re-invoke the built-in safely.** An override runs with `AGENTFORGE_DISABLE_OVERRIDES=1`, so calling `agentforge deploy` inside your wrapper hits the built-in (no infinite recursion).
- **Chaining is done in a wrapper script**, since `run:` is a single vector (not a shell line). To do "check, then the built-in", point `run:` at a script that sequences the steps:
  ...
  #!/usr/bin/env bash
  set -e
  "$AGENTFORGE_EXTENSION_DIR/scripts/compliance_check.sh"   # non-zero here aborts
  agentforge deploy "$@"                                  # hits the built-in (guard is set)
  ```
...
- **Conflicts** (same scope, shown in `agentforge extension list` / `agentforge info`): two extensions claiming one command is first-wins, later ignored. Cross-scope is fine — project wins over user (`--global`).
- **Declare a compatibility range** with `requires`, always. Run `agentforge --version`, set the lower bound to that `major.minor` and the upper bound to the next major (`>=1.3,<2` if the CLI reports 1.3.x — the number in the example above is only an example). Let the user pick `on_incompatible`; default `warn`.
  ...
  - `schema` (`agentforge-extension/v1alpha1`) tracks the manifest format, not the CLI version, and is accepted across CLI majors.
...
agentforge extension list                 # what's active, its scope, and its commands
agentforge extension update [<name>]      # advance the pin (re-resolve the tracked ref)
agentforge extension remove <name>        # drop it and delete its vendored copy
agentforge info                        # shows active extensions + sources + conflicts
```
...
A URL is cloned with your ambient git configuration, so whatever already authenticates you to that host (credential helper, ssh agent) applies — agentforge neither asks for nor stores credentials.

An extension changes commands. To run a different agent framework, scaffold from that framework's template instead (`agentforge create my-agent --agent <org>/<repo>@<tag>`); the template carries its own `agentforge-extension.yaml`, so nothing is installed machine-wide.

### Scopes

- **Project scope (default)** — committed to *this repo only* (recorded in `agentforge-extensions.yaml`, working copy vendored under `extensions/`), so it can't affect your other projects and CI/teammates get it. Commit both files so it works offline (`install` re-fetches if missing — see [Pinning and updates](#pinning-and-updates)).
...
- `extension add` resolves the ref to an exact commit SHA and records `source` / `ref` / `sha` under `extensions:` in `agentforge-extensions.yaml` (its own file at both scopes, so the writer can never touch your project manifest), vendoring a working copy under `extensions/`.
- `agentforge install` re-materializes any missing/stale vendored copy from the pinned SHA (verified by a stamp file) — CI and fresh checkouts get the exact reviewed code and command overrides. It never advances a pin.
...
```bash
agentforge extension add acme/acli-extensions#soc2 --ref v1.2.0   # pin a tag (recommended)
agentforge extension add acme/acli-extensions#soc2 --ref main     # follow a branch
agentforge extension update soc2                               # advance within the tracked ref
agentforge extension add acme/acli-extensions#soc2 --ref v1.3.0   # move to another tag: re-add, not update
agentforge extension add acme/acli-extensions#soc2 --ref v1.2.0   # roll back the same way
```
