...
> File paths below reference the scaffolded layout (`tests/eval/eval_config.yaml` or `.json`). Adjust for your project structure if not using `agentforge-scaffold`.
...
Run with `agentforge eval run --config tests/eval/eval_config.yaml`.
...
|-------|----------|-------------|
| `name` | yes | Unique identifier for the metric. |
| `prompt_template` | yes | Prompt template used by the judge model. With agentforge's file-based `EvaluationDataset` use `{prompt}`, `{response}`, and `{agent_data}` (the full trajectory). `{reference}` and `{context}` resolve only when the eval case has those fields populated. |
| `rubric_group_name` | n/a | **Rejected by agentforge.** It makes the service demand rubric verdicts a custom prompt cannot emit (`400 No rubric verdicts found in LLM response`). Grade `rubric_groups` with a managed metric plus `metric_spec_parameters.rubric_group_key`. |
