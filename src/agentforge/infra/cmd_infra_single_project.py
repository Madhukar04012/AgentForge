...
"""agentforge infra single-project — provision single-project terraform."""

import click

from agentforge._project import chdir_project_root
from agentforge._tools import require_tool
from agentforge.infra._terraform import require_single_project_tf_dir
...
def cmd_infra_single_project(project, apply_changes):
    ...
    Not required for basic deployments — `agentforge deploy` works out of the
    ...
    from agentforge.infra._cicd_utils import run_terraform
    ...
    if not apply_changes:
        followup = "agentforge infra single-project --apply"
        if project:
            followup += f" --project {project}"
        click.echo(f"\nTo apply these changes, run:\n  {followup}")
        return

    click.echo("\nTo see what was provisioned, run:\n  agentforge infra show")
