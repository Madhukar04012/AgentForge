...
"""AgentForge setup command — install skills via npx skills CLI.

Historically the package shipped as ``agentforge`` and is still installable
under that legacy name from PyPI (`uv tool install google-agents-cli`); the
newly-primary command is ``agentforge``.
"""
...
from agentforge._runner import run
from agentforge._skills_check import SKILLS_NPX_PACKAGE
from agentforge._tools import DEFAULT_SKILLS_SOURCE, run_npx_skills
from agentforge.skills._bundle import (
    SKILL_BUNDLE_DIR,
...
def _get_source_root():
    """Return the cwd if it is the agentforge repo root, else None.

    Checks that a ``pyproject.toml`` with the ``agentforge`` package
...
def _install_skills(remote_source, *, workspace, agent, allow_fallback):
    ...
    click.secho(
        "     'agentforge update' may not manage these copied skills; re-run "
        "'agentforge setup' once git and npx are available.",
        dim=True,
    )
...
def _check_legacy_skills():
    ...
    try:
        ...
        if found_legacy:
            ...
            click.secho(
                "     These may conflict with the new `agentforge` skills.",
                fg="yellow",
            )
            click.secho(
...
def cmd_setup(*, workspace, skip_auth, dry_run, dev, interactive, skills_source, agent):
    ...
    Installs the AgentForge tool (via uv tool install) and detects
    ...
    Use --dev to install AgentForge as editable from the local repo (for contributors).
    ...
    if dry_run:
        ...
        if dev:
            ...
            if not project_root:
                raise click.ClickException(
                    "--dev requires running from the root of the AgentForge repository"
                )
            click.echo("  Would install AgentForge (editable):")
            click.secho(
            ...
            click.echo("  Would install AgentForge:")
            click.secho(
        ...
        if not skip_auth:
            from agentforge.auth import is_authenticated
    ...
    step += 1
    if not skip_auth:
        from agentforge.auth import is_authenticated, run_auth_step
    ...
    if "already installed" in stdout.lower() or "already installed" in stderr.lower():
        from agentforge.scaffold.utils.version import check_for_updates
        ...
        click.secho("  Could not install AgentForge automatically.", fg="yellow")
        if stderr.strip():
    ...
    if not workspace:
        from agentforge.setup._antigravity import link_skills_for_antigravity
    ...
    if cli_installed:
        if dev:
            click.echo("  CLI:    agentforge installed (editable)")
        else:
            click.echo("  CLI:    agentforge installed")
    else:
