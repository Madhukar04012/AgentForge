...
"""AgentForge eval run command — chain generate + grade in one step."""
...
from agentforge.eval.cmd_generate import (
    _DEFAULT_APP_NAME,
    _DEFAULT_CONCURRENCY,
    cmd_generate,
)
from agentforge.eval.cmd_grade import cmd_grade, qps_option
...
def cmd_run(
    *,
    ...
    \b
    Example:
      agentforge eval run --dataset eval_cases.json --metrics final_response_quality
    """
    console = Console()
    ...
    if not project_root:
        raise click.ClickException(
            "Not inside an agentforge project (no manifest found)."
        )
    ...
    from agentforge.extension._overrides import installed_override, run_override
