...
"""AgentForge eval dataset commands — synthesize evaluation traces."""
...
_SYNTHESIZE_STAGE_DIR = ".agentforge-scripts"
# Legacy pre-rename stage dir, kept for one release so a project that ran
# ``agentforge eval dataset generate`` under the old name still works.
_SYNTHESIZE_STAGE_DIR_LEGACY = ".agents-cli-scripts"
...
def _stage_synthesize_runner(dest_dir: Path) -> Path:
    ...
    agentforge is installed normally, in editable mode, or as a wheel.
    ...
    """
    pkg = resources.files("agentforge.eval")
...
@click.option(
    ...
    help=(
        ...
        "`agentforge eval grade` can consume it directly."
    ),
)
def cmd_synthesize(
    count: int,
    ...
    *,
    ...
    ADK agent (read from `agent_directory` in agentforge-manifest.yaml) and running it
    ...
    Examples:
      ...
      agentforge eval dataset synthesize -n 10 --max-turns 8 \\
        --instruction "Scenarios where users change destination"

    \b
    Requires:
      - A local ADK agent project with agentforge-manifest.yaml
      - Google Cloud authentication (`agentforge login`)
    """
    ...
    # One-shot migration: if the new stage dir is missing but the legacy
    # pre-rename dir is present, prefer the legacy path so the user's
    # last run can still be inspected. The next run will write to the new
    # path; the legacy dir is then left in place but unused.
    if not stage_dir.exists():
        legacy_stage_dir = project_root / _SYNTHESIZE_STAGE_DIR_LEGACY
...
dataset_group.add_lazy_command(
    "synthesize",
    "agentforge.eval.cmd_dataset:cmd_synthesize",
    "[Experimental] Synthesize evaluation traces (eval cases with full agent runs).",
)
