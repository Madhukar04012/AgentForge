...
"""AgentForge eval generate command — run agent inference over dataset."""
...
from agentforge._adk_client import create_session, fetch_app_info, run_sse
...
from agentforge._remote import build_remote_headers
from agentforge.eval import _paths
from agentforge.run._local_server import ensure_server, stop_server
...
@click.option(
    ...
    help=(
        ...
        "file scaffolded by `agentforge create`)."
    ),
)
@click.option(
    "--output",
...
@click.option(
    ...
    help=(
        ...
        "omitted, agentforge runs the agent in a local HTTP server. In both "
...
def cmd_generate(
    *,
    ...
    downstream scoring with `agentforge eval grade`. Eval cases run in parallel.
    ...
    \b
    Example:
      agentforge eval generate --dataset eval_cases.json --output artifacts/traces/
