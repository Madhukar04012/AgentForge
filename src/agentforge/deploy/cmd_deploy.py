...
"""AgentForge deploy command — deploy the agent."""
...
from agentforge._agent_platform import AgentPlatformClient
from agentforge._gcp_project import resolve_gcp_project
...
from agentforge._runner import popen_resolved, run, run_resolved
from agentforge.auth import get_access_token
from agentforge.deploy._utils import (
    DEFAULT_CONCURRENCY,
    DEFAULT_CPU,
...
from agentforge.deploy.agent_runtime import (
    check_agent_runtime_operation,
    deploy_agent_runtime,
    parse_secrets,
)
from agentforge.scaffold.utils.language import get_project_version
...
def _load_deploy_config(
    ...
    if project_root is None and deployment_target is None:
        raise click.ClickException(
            "No agentforge-manifest.yaml found in the current directory or its parents.\n"
...
@click.option(
    ...
    default=None,
    help="Deployment target. Overrides agentforge-manifest.yaml and lets deploy "
...
def cmd_deploy(
    *,
    ...
    \b
    Dispatches by deployment target configured in agentforge-manifest.yaml:
    ...
    manifest (e.g. from a built container or CI):
      agentforge deploy --deployment-target cloud_run

    \b
    Use --list to show existing deployments:
      agentforge deploy --list
    ...
    if not has_manifest:
        ...
        logging.warning(
            "No agentforge-manifest.yaml found — deploying with defaults:\n"
            "    • service name:    %s\n"
            "    • agent directory: %s\n"
            "    • building from:   %s\n"
            "  Pass --service-name to set the service name, run from a scaffolded "
            "project, or see `agentforge deploy --help` for all flags.",
            service_name,
            cfg.agent_directory,
            os.getcwd(),
        )
    ...
    if cfg.deployment_target == "agent_runtime":
        ...
        if parsed_labels.get("created-by") not in (None, "agentforge"):
            logging.warning(
                "Ignoring --labels created-by=%s: 'created-by' is currently reserved.",
                parsed_labels["created-by"],
            )
        cr_labels = {**parsed_labels, "created-by": "agentforge"}
        ...
        raise click.ClickException(
            ...
            "Set deployment_target in agentforge-manifest.yaml."
        )
...
def _list_agent_runtime_deployments(project: str | None, location: str) -> None:
    ...
    from agentforge.auth import get_adc_credentials
