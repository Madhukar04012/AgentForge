...
from agentforge._remote import build_agent_runtime_passthrough_url
from agentforge.deploy._operation import (
    METADATA_FILE,
    clear_operation,
    read_operation,
    write_operation,
...
from agentforge.scaffold.utils.language import (
    get_language_config,
    get_project_version,
)
    ...
    *, agent: Any, agent_identity: bool, display_name: str
    ...
    if has_agent_identity and not agent_identity:
        ...
        raise click.ClickException(
            ...
            f"    • Delete the existing agent and redeploy:  agentforge deploy "
            "--agent-identity"
        )
...
def _missing_dockerfile_error(cfg: ProjectConfig) -> str:
    ...
    if scaffold_older_than(cfg, _CONTAINER_RUNTIME_VERSION):
        ...
        lines += [
            "",
            f"  This project was scaffolded with agentforge {version}, before",
            "  agent_runtime used containers. Either:",
            "    • migrate the project:  agentforge scaffold upgrade",
            "    • or deploy with the version it was built for:",
            f"        uvx google-agents-cli@{version} deploy",
        ]
    else:
        lines += [
            "  Run `agentforge scaffold upgrade` to regenerate it, or recreate",
            "  the project with `agentforge create`.",
        ]
    return "\n".join(lines)
    ...
    *,
    ...
    if update_only and not is_update:
        raise click.ClickException(
            ...
            "  Create it first (for example with `agentforge infra single-project "
    ...
    if no_wait:
        click.echo("   Check status with: agentforge deploy --status")
        return None
    click.echo(
        "   If this command is interrupted, run 'agentforge deploy --status' to check progress."
    )
...
def check_agent_runtime_operation(
    ...
    if not op_data:
        raise click.ClickException(
            ...
            "  Run 'agentforge deploy' or 'agentforge deploy --no-wait' first."
        )
    ...
    if operation.done:
        ...
        click.echo(
            ...
            "   Run 'agentforge deploy --status' again to check."
        )
