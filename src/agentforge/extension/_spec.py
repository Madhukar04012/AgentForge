...
"""Parse and validate agentforge-extension.yaml (schema: agents-cli-extension/v1alpha1).

The on-disk filename and the schema URL are kept under the
``agents-cli-extension/...`` wire name so existing extensions keep resolving
the same schema; only the parsed field on disk is renamed to ``requires.agentforge``
with the legacy ``requires.agents_cli`` still accepted for one release.
"""
...
from agentforge.extension._compat import validate_range

# The manifest format version, tracked separately from the agentforge version.
...
V1ALPHA1_ALIAS = "agentforge-extension/v1alpha1"
EXTENSION_FILE = "agentforge-extension.yaml"
...
# --- Wire models -------------------------------------------------------------
# These mirror agentforge-extension.yaml and generate the published JSON Schema.
...
class Requires(_Strict):
    # `agentforge: 1.2` is the new primary key. The legacy `agents_cli:` is
    # accepted as a fallback so existing extensions keep loading; if both are
    # set, ``agentforge`` wins (``_parse_requires`` enforces precedence).
    agentforge: Annotated[str | float | None, BeforeValidator(_reject_bool)] = None
...
class ExtensionManifest(_Strict):
    ...
    # anything else. The legacy wire name is kept as an alias for one
    # release so existing extensions keep loading.
    schema_version: Literal["agents-cli-extension/v1alpha1", "agentforge-extension/v1alpha1"] = Field(
...
class ExtensionSpec:
    ...
    scope: str = ""
    # Optional author-declared agentforge compatibility range (e.g. ">=1.1,<2").
...
def _parse_requires(requires: Requires | None) -> tuple[str | None, bool]:
    """Resolve the CLI compat range from the ``requires:`` block.

    Resolution order:
      1. ``requires.agentforge`` (new primary)
      2. ``requires.agents_cli`` (legacy compat alias)

    An unparseable range is warned about and dropped: a bad compat hint must
    not make an otherwise-valid extension unloadable.
    """
    if requires is None:
        return None, False
    raw_range = None
    if requires.agentforge is not None:
        raw_range = str(requires.agentforge)
    elif requires.agents_cli is not None:
        raw_range = str(requires.agents_cli)
    if raw_range is not None and not validate_range(raw_range):
        logging.warning(
            "requires.agentforge %r is not a valid version range; ignoring the "
