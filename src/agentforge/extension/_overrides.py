...
from agentforge._runner import DISABLE_OVERRIDES_ENV, run_extension_command
from agentforge.extension._spec import ExtensionCommand

if TYPE_CHECKING:
    from agentforge.extension._loader import ResolvedCommand
...
def installed_override(dotted: str) -> ResolvedCommand | None:
    ...
    if resolved.blocked_requires:
        raise click.ClickException(
            ...
            f"requires agentforge {resolved.blocked_requires}.\n"
            f"  Update it:      agentforge extension update {resolved.extension_name}\n"
            f"  Or remove it:   agentforge extension remove {resolved.extension_name}"
        )
    return resolved
...
def run_override(
    ...
    if code != 0:
        # Exit with the child's code, matching what a direct `agentforge <cmd>`
...
def bypass_hint(display_path: str) -> str:
    ...
    """
    if os.name == "nt":
        return f"set {DISABLE_OVERRIDES_ENV}=1 && agentforge {display_path}"
    return f"{DISABLE_OVERRIDES_ENV}=1 agentforge {display_path}"
...
def make_blocked_command(
    *,
    ...
    """
    short = f"unavailable: extension '{extension_name}' needs agentforge {requires}"
    ...
    @click.command(
        ...
        help=(
            ...
            f"requires agentforge {requires}; you are running {running}. The "
            ...
            "built-in explicitly:\n"
            f"  agentforge extension update {extension_name}\n"
            f"  agentforge extension remove {extension_name}\n"
    ...
    def _blocked(extra_args: tuple[str, ...]) -> None:
        raise click.ClickException(
            ...
            f"agentforge {requires} (running {running}).\n"
            f"  Update it:      agentforge extension update {extension_name}\n"
            f"  Or remove it:   agentforge extension remove {extension_name}\n"
