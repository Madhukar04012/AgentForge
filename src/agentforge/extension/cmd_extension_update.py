...
"""agentforge extension update command."""
...
from agentforge.extension._loader import incompatible_spec
...
from agentforge.extension._spec import ExtensionSpecError
        ...
        for entry in read_extension_entries(manifest):
            ...
            if spec is not None:
                if spec.error_on_incompatible:
                    _restore_pin(entry, root)
                    click.secho(
                        f"Skipped {entry.name!r}: new version requires agentforge "
                    ...
                    continue
                logging.warning(
                    "Updated %r to a version requiring agentforge %s (running %s).",
                    entry.name,
