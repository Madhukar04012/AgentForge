...
"""agentforge extension add command."""
...
from agentforge._trust import require_confirmation
from agentforge.extension._compat import is_compatible
from agentforge.extension._loader import (
    RECOVERY_COMMANDS,
    load_extension_set,
    load_extension_spec,
)
from agentforge.extension._manifest import (
    EXTENSIONS_FILE,
    ExtensionEntry,
    read_extension_entries,
    upsert_extension_entry,
)
from agentforge.extension._paths import (
    scope_root,
    user_config_root,
    vendored_extensions_dir,
)
from agentforge.extension._refs import RefParseError, parse_ref
from agentforge.extension._resolver import ResolverError, materialize, resolve_sha
from agentforge.extension._spec import (
    EXTENSION_FILE,
    ExtensionSpec,
    ExtensionSpecError,
)
from agentforge.extension._trust import confirm_trust
...
def _check_compat(spec: ExtensionSpec | None) -> None:
    ...
    if (
        ...
        return
    msg = (
        f"Extension requires agentforge {spec.requires_agents_cli} but you're "
...
def cmd_add(
    ...
    if root is None:
        raise click.ClickException(
            "No agentforge-manifest.yaml found. Run inside a project (extensions "
    ...
    try:
        ...
        if any(
            ...
            raise click.ClickException(
                f"{e.format_message()}\n"
                f"  {name!r} is still recorded; restore its copy with: agentforge install"
            ) from e
        raise
