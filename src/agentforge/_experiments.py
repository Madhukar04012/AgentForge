...
def resolve_experiment(label: str) -> Any:
    ...
    # Read the new name first; fall back to the legacy name for one release so
    # CI scripts that set the old name keep working.
    env_val = os.environ.get("AGENTFORGE_EXPERIMENTS")
    if env_val is None:
    ...
    if env_val:
        try:
            ...
            logging.warning(
                f"Failed to apply AGENTFORGE_EXPERIMENTS override for '{label}', "
