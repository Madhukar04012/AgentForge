...
"""Root Click group for the 'agentforge' CLI.
...
from agentforge.__init__ import __version__
from agentforge._click import LazyGroup, patch_source_in_help
from agentforge._project import find_project_root, is_project_moved
...
# that bypasses overrides (AGENTFORGE_DISABLE_OVERRIDES=1 — set for any command
...
if TYPE_CHECKING:
    from agentforge.extension._loader import ExtensionSet, ResolvedCommand
...
def _print_is_project_moved_tip() -> None:
    message = (
        ...
        f" Try running `{_prog_name()} install --clean` to reset the environment, then"
        " re-run your original command"
    )
    if is_project_moved():
        from agentforge._output import Console
...
class _MainGroup(LazyGroup):
    ...
    def _apply_extensions(self) -> None:
        ...
        # (AGENTFORGE_DISABLE_OVERRIDES=1, e.g. an override re-entering the CLI)
        # never loads the extension discovery code.
        if read_disable_overrides() == "1":
            return

        from agentforge.extension._loader import load_extension_set
        from agentforge.extension._paths import user_config_root
        ...
        for inc in extension_set.incompatible:
            if inc.error_on_incompatible:
                logging.warning(
                    "agentforge: extension %r requires AgentForge %s but running %s; "
                    "its commands will fail until you run `agentforge extension "
        ...
        if applied:
            ...
            logging.warning(
                "agentforge: applying %d extension command(s): %s",
                len(applied),
                ", ".join(details),
            )
    ...
    def _synthesize(self, resolved: ResolvedCommand, *, leaf: str, display: str):
        ...
        """
        from agentforge.extension._overrides import (
    ...
    def invoke(self, ctx: click.Context) -> None:
        try:
            ...
            click.echo(f"{_prog_name()} v{__version__}", err=True)
            ...
            console = Console()
            console.print(f"\n{_prog_name()} v{__version__}", style="dim")
...
def _prog_name() -> str:
    """Return the user-facing program name for the running invocation.

    Both `agentforge` (new) and `agents-cli` (legacy) resolve to this same
    `main()` function via their respective console scripts. We derive the
    program name from the actual script invocation so --help and --version
    show the name the user typed, not a hardcoded one.
    """
    script = os.path.basename(sys.argv[0]) if sys.argv else "agentforge"
    # Strip any `.exe` suffix added by Windows wrapper launchers.
    if script.lower().endswith(".exe"):
        script = script[:-4]
    # Map the legacy console script to the new primary name when invoked via
    # the new entry point; keep the literal script name otherwise so messages
    # from the legacy binary still self-identify as `agents-cli`.
    return script or "agentforge"


@click.group(cls=_MainGroup)
@click.version_option(version=__version__, prog_name=_prog_name())
def main():
    ...
    This is the same toolchain historically known as the Google Agents CLI;
    it ships as the ``agentforge`` command, with ``agents-cli`` retained
    as a compatibility alias.

    \b
    Quick start:
      agentforge setup                 Install skills to your coding agent
      agentforge create my-agent       Create a new agent project
      agentforge playground            Start the local playground
      agentforge eval run              Run agent inference and grade the traces
      agentforge scaffold enhance .    Add deployment/CI-CD to a project
      agentforge deploy                Deploy the agent
    """
    ...
    from agentforge._skills_check import check_skills_version
    from agentforge.scaffold.utils.version import display_update_message
...
main.add_lazy_command(
    "setup",
    "agentforge.setup.cmd_setup:cmd_setup",
    "Install AgentForge and skills to detected coding agents.",
)
main.add_lazy_command(
    "update",
    "agentforge.setup.cmd_update:cmd_update",
    "Force reinstall AgentForge skills to all detected coding agents.",
)

# Auth commands
main.add_lazy_command(
    "login",
    "agentforge.setup.cmd_auth:cmd_login",
...
main.add_lazy_command(
    "scaffold",
    "agentforge.scaffold.cmd_scaffold_group:scaffold_group",
...
main.add_lazy_command(
    "playground",
    "agentforge.dev.cmd_playground:cmd_playground",
    "Start the local agent playground.",
)
main.add_lazy_command(
    "run",
    "agentforge.run.cmd_run:cmd_run",
    "Run the agent with a single prompt (non-interactive).",
)
main.add_lazy_command(
    "lint",
    "agentforge.dev.cmd_lint:cmd_lint",
    "Run code quality checks.",
)
main.add_lazy_command(
    "install",
    "agentforge.dev.cmd_install:cmd_install",
    "Install project dependencies.",
)
main.add_lazy_command(
    "build",
    "agentforge.dev.cmd_build:cmd_build",
...
main.add_lazy_command(
    "data-ingestion",
    "agentforge.data.cmd_data_ingestion:cmd_data_ingestion",
...
main.add_lazy_command(
    "eval",
    "agentforge.eval.cmd_eval_group:eval_group",
...
main.add_lazy_command(
    "deploy",
    "agentforge.deploy.cmd_deploy:cmd_deploy",
    "Deploy the agent.",
)
main.add_lazy_command(
    "publish",
    "agentforge.publish.cmd_publish_group:publish_group",
...
main.add_lazy_command(
    "infra",
    "agentforge.infra.cmd_infra:infra_group",
...
main.add_lazy_command(
    "extension",
    "agentforge.extension.cmd_extension_group:extension_group",
    "Manage AgentForge extensions (experimental).",
)

# Info command
main.add_lazy_command(
    "info",
    "agentforge.info.cmd_info:cmd_info",
