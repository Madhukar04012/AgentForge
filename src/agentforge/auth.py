...
"""Authentication module for AgentForge.
...
def run_auth_step(show_header=True):
    ...
    if choice == 3:
        click.echo()
        click.secho(
            "  Skipped. Run 'agentforge login' later to authenticate.",
            fg="yellow",
        )
        click.echo()
        return True
    ...
    if result is not None:
        ...
        click.secho(
            "  Continuing without authentication. Run 'agentforge login' later.",
            fg="yellow",
        )
...
def get_access_token() -> str:
    ...
    from agentforge.scaffold.utils.command import run_gcloud_command
