...
import agentforge as _cli_pkg
from agentforge.__init__ import __version__
from agentforge._output import emit
from agentforge._project import (
    check_cli_version,
    find_project_root,
    read_project_config,
)
from agentforge._skills_check import get_installed_skills
from agentforge.extension._loader import ExtensionSet, load_extension_set
...
def _print_extensions(extension_set: ExtensionSet) -> None:
    ...
    for r in rows:
        ...
        if r.get("requires"):
            line += f"  [requires agentforge {r['requires']}]"
        if r.get("blocked"):
    ...
    for inc in incompatible:
        click.echo(
            f"  ! incompatible: {inc['extension']} requires agentforge "
...
def cmd_info(as_json: bool) -> None:
    ...
    if project_root is None:
        if as_json:
            ...
            click.echo("    agentforge create my-agent")
        return
