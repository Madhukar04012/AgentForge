...
from agentforge._tools import ToolNotFoundError, require_tool
...
def _ensure_uvx_available(project_version: str) -> None:
    ...
    try:
        ...
        console.print(
            f"❌ Project requires agentforge version {project_version}, "
...
def _execute_with_saved_config(
    ...
    if use_different_version and project_version:
        console.print(
            f"📦 Using agentforge version {project_version}...",
            style="dim",
        )
        _ensure_uvx_available(project_version)
        cmd = ["uvx", f"agentforge@{project_version}", *args]
    else:
        ...
        cmd = [sys.executable, "-m", "agentforge.main", *args]
...
def enhance(
    ...
    *,
    ...
    If the project has an agentforge-manifest.yaml, the template recorded there
    is re-applied and TEMPLATE_PATH is ignored. Pass a local directory or a
    remote spec (org/repo@tag) for a project agentforge did not create.
    ...
    if base_template and not validate_base_template(base_template):
        hint = (
            f"  To apply a fetched template, pass it positionally: agentforge scaffold enhance {base_template}"
