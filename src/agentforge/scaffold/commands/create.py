...
def shared_template_options(f: Callable) -> Callable:
    ...
    f = click.option(
        ...
        help=(
            ...
            "root CA (Agent Runtime only). Required before `agentforge deploy "
...
def create(
    ...
    *,
    ...
    from agentforge.deploy._utils import validate_deployment_region
    ...
    if final_deployment == "none":
        console.print(
            "\n[bold cyan]💡 Tip[/]\n"
            "   Add a deployment target later with: [cyan]agentforge scaffold enhance[/]"
        )
    elif final_cicd_runner == "skip":
        console.print(
            "\n[bold cyan]💡 Tip[/]\n"
            "   Once ready for production, run: [cyan]agentforge scaffold enhance[/]"
        )
    ...
    console.print(
        f"   [bold bright_green]cd {cd_path} && agentforge install && agentforge playground[/]"
    )
