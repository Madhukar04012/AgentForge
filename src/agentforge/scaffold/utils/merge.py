...
def run_create_command(
    ...
    if not _is_fetchable_version(version):
        logging.warning(
            "Cannot fetch agentforge@%s to build the prior-version baseline; "
    ...
    logging.warning(
        "Could not build the agentforge@%s baseline via uvx; falling back to "
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            console.print(
                "[dim] Dependencies updated — run [bold]agentforge install[/bold] "
