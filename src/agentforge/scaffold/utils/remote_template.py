...
def check_and_execute_with_version_lock(
    ...
    """Check if remote template has agentforge version lock.

    Since the ACLI code is vendored into agentforge, version locking
    ...
    if version:
        logging.debug(
            f"Remote template specifies agentforge version {version}; "
...
def fetch_remote_template(
    ...
    with agentforge version constraint, will execute nested uvx command.
    ...
    try:
        ...
        from agentforge._runner import run_resolved
...
def load_remote_template_config(
    ...
    agentforge-manifest.yaml, or legacy [tool.agentforge] / [tool.agents-cli] section in pyproject.toml
    with CLI overrides.
    ...
    # Load from .template/templateconfig.yaml, agentforge-manifest.yaml, or pyproject.toml
    template_config_path = template_dir / ".template" / "templateconfig.yaml"
    manifest_path = template_dir / "agentforge-manifest.yaml"
    legacy_manifest_path = template_dir / "agents-cli-manifest.yaml"
    pyproject_path = template_dir / "pyproject.toml"

    # If the new manifest is missing but the legacy one is present, use the
    # legacy one and fold it forward.
    if not manifest_path.exists() and legacy_manifest_path.exists():
        legacy_manifest_path.rename(manifest_path)
    ...
    if template_config_path.exists():
        ...
        try:
            ...
            if manifest_config:
                ...
                logging.debug("Found explicit agentforge-manifest.yaml configuration")
        except Exception as e:
            logging.error(f"Error loading agentforge-manifest.yaml config: {e}")
    elif pyproject_path.exists():
        try:
            with open(pyproject_path, "rb") as f:
                pyproject_data = tomllib.load(f)

            # Extract the agentforge configuration. New primary key is
            # ``[tool.agentforge]``; the legacy ``[tool.agents-cli]`` is a
            # fallback.
            toml_config = (
                pyproject_data.get("tool", {}).get("agentforge")
                or pyproject_data.get("tool", {}).get("agentforge")
                or {}
            )

            # Fallback to [project] fields if not specified in agentforge section
            ...
            if toml_config:
                ...
                logging.debug("Found explicit [tool.agentforge] configuration")
        ...
        if is_adk_sample:
            ...
            logging.warning(
                "%s declares no agentforge configuration, so it is treated as an ADK "
...
def parse_acli_version_from_lock(
    ...
    """Parse agentforge version from uv.lock file.
    ...
    try:
        ...
        # Look for agentforge in the packages section
        packages = lock_data.get("package", [])
        for package in packages:
            if package.get("name") == "google-agents-cli":
                version = package.get("version")
                if version:
                    logging.debug(f"Found agentforge version {version} in uv.lock")
                    return version
