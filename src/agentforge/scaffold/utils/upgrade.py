...
def write_python_dependencies(
    ...
    try:
        ...
        from agentforge._tools import ToolNotFoundError
...
def update_acli_metadata(
    ...
    *,
    ...
    """Update specific keys in the unified agentforge-manifest.yaml metadata file.
    ...
    """
    manifest_path = project_dir / "agentforge-manifest.yaml"
    legacy_manifest_path = project_dir / "agents-cli-manifest.yaml"
    if not manifest_path.exists() and legacy_manifest_path.exists():
        # One-shot fold: rename the legacy filename forward so subsequent
        # reads find the new manifest. (We do not write a new manifest
        # here — this function only edits an existing one.)
...
def migrate_legacy_python_config(
    ...
    """Extract legacy ``[tool.agents-cli]`` config from pyproject.toml and write to ``agentforge-manifest.yaml``.

    The new primary pyproject key is ``[tool.agentforge]``; this function is
    a no-op when neither legacy nor new key is present. The legacy
    ``agents-cli-manifest.yaml`` filename is also folded forward: if it
    exists when the new manifest is being written, it is renamed in place.
    """
    pyproject_path = project_dir / "pyproject.toml"
    manifest_path = project_dir / "agentforge-manifest.yaml"
    legacy_manifest_path = project_dir / "agents-cli-manifest.yaml"
    ...
    if not pyproject_path.exists():
        # Still fold the legacy manifest filename forward if it exists.
    ...
    acli = tool_section.get("agentforge")
    if acli is None:
        return
    ...
    if manifest_path.exists() or legacy_manifest_path.exists():
        ...
        click.secho(
            "  ▸ Legacy [tool.agents-cli] section found in pyproject.toml but "
            "agentforge-manifest.yaml already exists. The legacy config "
            "will be ignored and that section can be safely deleted.",
            fg="yellow",
            dim=True,
        )
        return
    ...
    if dry_run:
        ...
        click.secho(
            "  ▸ [Dry run] Legacy pyproject.toml configuration would be migrated to agentforge-manifest.yaml",
            fg="yellow",
            dim=True,
        )
        return
    ...
    click.secho(
        "  ▸ Legacy pyproject.toml configuration successfully migrated to agentforge-manifest.yaml",
        fg="cyan",
        dim=True,
    )
...
def migrate_legacy_evalsets(project_dir: pathlib.Path, dry_run: bool = False) -> None:
    ...
    if converted:
        ...
        click.secho(
            ...
            "Run `agentforge eval generate` to populate the trace.",
            fg="cyan",
            dim=True,
        )
    if skipped:
