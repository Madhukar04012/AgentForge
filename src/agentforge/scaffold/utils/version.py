...
# Legacy PyPI distribution name; ``version()`` is queried against the new name
# first and falls back to the legacy name so a pre-rename install keeps being
# recognised as the same product.
PACKAGE_NAME_LEGACY = "google-agents-cli"
...
# ``.agentforge_update_check`` carries the new project name; the parent
# ``~/.agents/`` stays as-is because that path is the industry convention.
_UPDATE_CHECK_STAMP = Path.home() / ".agents" / ".agentforge_update_check"
...
def get_current_version() -> str:
    ...
    Tries the new distribution name first; falls back to the legacy name so
    a pre-rename install still reports a useful version.
    """
    try:
        return version(PACKAGE_NAME)
    except PackageNotFoundError:
        try:
            return version(PACKAGE_NAME_LEGACY)
        except PackageNotFoundError:
            # Package isn't installed (editable / dev checkout).
            return UNKNOWN_VERSION


def agentforge_version_pin() -> str:
    """Return the version suffix to pin `agentforge` in generated projects.
    ...
    package leave the deploy steps using an unpinned `agentforge`.
    """
...
# Backward-compat alias: callers that imported the old function name keep
# working. Kept for one release.
agents_cli_version_pin = agentforge_version_pin
