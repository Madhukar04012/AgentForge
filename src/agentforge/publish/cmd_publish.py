...
from agentforge._gcp_project import (
    get_gcp_project_number,
    resolve_gcp_project,
)
from agentforge._output import Console, emit
from agentforge._project import read_project_config
...
from agentforge.auth import get_access_token, get_id_token
from agentforge.scaffold.utils.gcp import (
    get_user_agent,
    get_x_goog_api_client_header,
)
from agentforge.scaffold.utils.language import get_language_config
from agentforge.scaffold.utils.logging import display_welcome_banner
...
def _run_sdk_upgrade() -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            console.print(
                "  1. Redeploy your agent to pick up the fix: [cyan]agentforge deploy[/cyan]"
            )
...
def register_gemini_enterprise(
    *,
    ...
    Use --list to list Gemini Enterprise apps in the current project:
      agentforge publish gemini-enterprise --list
    """
    if list_apps:
