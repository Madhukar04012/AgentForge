...
> **Assumes `/agentforge-scaffold` scaffolding.** Observability infrastructure is provisioned by Terraform in scaffolded projects.
...
No configuration required. Works in local dev (`agentforge playground`) and all deployed environments.
...
Telemetry config is env-var driven, so set the same vars Terraform sets for deployed agents before running `agentforge playground`:
...
PROJECT_NAME="your-app-name"  # The agentforge project name (not the GCP project ID)
