...
description: >
  ...
  "publish to Gemini Enterprise", or needs guidance on the agentforge
...
> **Requires:** A deployed agent. For Agent Runtime, `deployment_metadata.json` (created by `agentforge deploy`) enables auto-detection. For Cloud Run or GKE, provide the agent card URL and flags directly.
...
3. **`deployment_metadata.json`** (Agent Runtime only) — Created automatically by `agentforge deploy`; contains the agent runtime ID, deployment target, the A2A flag, and the agent directory
...
```bash
agentforge publish gemini-enterprise --interactive
```
...
Agent Platform) are **auto-registered** — no extra step after `agentforge deploy`.
...
- `/agentforge-deploy` — Deployment targets, CI/CD pipelines, and production workflows (also covers Agent Gateway governed ingress/egress and Semantic Governance awareness)
