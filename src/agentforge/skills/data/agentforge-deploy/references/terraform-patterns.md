...
> **Assumes `/agentforge-scaffold` scaffolding.** These patterns apply to projects with `deployment/terraform/` directories.
...
agentforge infra single-project  # Runs terraform apply in deployment/terraform/single-project/
...
**Cloud SQL sessions:**
- Already configured when using `--session-type cloud_sql` via the AgentForge (see `/agentforge-scaffold`)
