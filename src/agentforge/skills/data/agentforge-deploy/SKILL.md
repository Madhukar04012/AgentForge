...
description: >
  ...
  Applies to any framework agentforge deploys (ADK, LangChain, ...).
  Part of the agentforge skills suite.
  Do NOT use for agent API code patterns (ADK: use agentforge-adk-code), evaluation
  (use agentforge-eval), or project scaffolding (use agentforge-scaffold).
metadata:
  author: Google
  license: Apache-2.0
  version: 1.5.0
...
> **Requires:** `agentforge` (`uv tool install agentforge`) — [install uv](https://docs.astral.sh/uv/getting-started/installation/index.md) first if needed.

> Prefer using the `agentforge` commands throughout this guide — they wrap Terraform, Docker, and deployment into a tested pipeline. If your project isn't scaffolded yet, see `/agentforge-scaffold` to add deployment support first.
...
- **`batch-inference.md`** — BigQuery Remote Function trigger; for Pub/Sub / Eventarc on ADK see `/agentforge-adk-code`
...
> **Observability:** See the `/agentforge-observability` skill for Cloud Trace, prompt-response logging, BigQuery Analytics, and third-party integrations.
...
| **Batch/event processing** | Trigger endpoints reachable via the Agent Engine `/api` passthrough | Native trigger endpoints (Pub/Sub, Eventarc); ADK: see `/agentforge-adk-code` | Custom (Kubernetes Jobs, Pub/Sub) |
...
1. If prototype (no deployment target), first enhance: `agentforge scaffold enhance . --deployment-target <target>`
2. **Notify the human**: paste the eval scores and test results, then ask "Ready to deploy to dev?"
3. **Wait for explicit approval**
4. Once approved: `agentforge deploy`

> **Agent Runtime timeout recovery:** Agent Runtime deploys can take 5-10 minutes and may exceed command timeouts. If the deploy command is cancelled or times out, the deployment continues server-side. Run `agentforge deploy --status` to check progress — poll every 60 seconds until it reports completion or failure.

**IMPORTANT**: Never run `agentforge deploy` without explicit human approval.

> **Do NOT run `agentforge infra single-project` before deploying.** It is not a prerequisite — `agentforge deploy` works on its own. Run it separately if the user needs observability features (prompt-response logging, BigQuery analytics) — see `/agentforge-observability`.

### Single-Project Infrastructure Setup (Optional — Advanced)

`agentforge infra single-project` runs `terraform apply` in `deployment/terraform/single-project/`. Use this to **provision single-project GCP infrastructure without CI/CD** (service accounts, IAM bindings, telemetry resources, Artifact Registry). Also useful to test things in a single project before going to production. It is NOT required for deploying.

```bash
# Optional — provision infrastructure in a single GCP project
agentforge infra single-project
```

> **Note:** `agentforge deploy` doesn't automatically use the Terraform-created `app_sa`. Pass the service account explicitly: `agentforge deploy --service-account SA_EMAIL`.
...
Run `agentforge deploy --help` for the full flag reference.

> **Advanced Cloud Run Deploys:** If you need features not exposed via `agentforge` flags, use `--dry-run` (or `-n`) to print the full `gcloud` command, copy it, and add additional arguments as needed.
...
`agentforge deploy` scales to zero by default so idle dev and demo agents don't hold capacity. Pass `--min-instances 1` (or deploy via Terraform) when you need a warm instance.
...
agentforge deploy --cpu 4 --concurrency 16 --memory 16Gi --max-instances 20
```
...
> **ADK projects.** For event-driven / ambient agent deployment on Cloud Run, see the [`ambient-expense-agent`](https://github.com/google/adk-samples/tree/main/core/python/ambient-expense-agent) sample and `/agentforge-adk-code` (`references/adk-python.md`, section "12. Event-Driven / Ambient Agents") for the `trigger_sources` pattern.

---

## Agent Runtime Specifics

Agent Runtime is a managed Vertex AI service for deploying agents as containers. Uses container-based deployment: `agentforge deploy` packages your project and Agent Engine builds the image from your project's `Dockerfile` (required) — the same `fast_api_app:app` image that serves Cloud Run and GKE.

> **No `gcloud` CLI exists for Agent Runtime.** Deploy via `agentforge deploy`. Query via the Python `agentplatform.Client` SDK.
...
agentforge deploy --no-wait

# Check on progress later
agentforge deploy --status
```
...
```bash
agentforge deploy --secrets "API_KEY=my-api-key,DB_PASS=db-password:2"
```
...
The quickest way to test a deployed agent is `agentforge run --url <service-url> --mode a2a "your prompt"` — it handles auth, sessions, and streaming automatically (supports Agent Runtime and Cloud Run). **ADK:** `--mode adk` talks to the ADK streaming API instead.
...
IAP (Identity-Aware Proxy) secures a Cloud Run service so only authorized Google accounts can access it. Enable it by adding the `--iap` flag when deploying (Cloud Run only): `agentforge deploy --iap`.
...
Agent Runtime doesn't support revision-based rollback — fix and redeploy via `agentforge deploy`.
...
For registering deployed agents with Gemini Enterprise, see `/agentforge-publish`.

---

## Agent Gateway

> **Note:** There are no `agentforge` commands for creating or managing Agent Gateways or
> Semantic Governance policies — set those up separately (via Terraform or the Cloud Console).
> `agentforge deploy` is the only gateway-aware command, and its support is a passthrough: it
...
Once a gateway exists, `agentforge deploy` binds an agent to it with `--agent-gateway-egress`
...
creating a new project, pass `--agent-gateway` flag to `agentforge create`:

```bash
agentforge create my-agent -d agent_runtime --agent-gateway
```

Alternatively, you can pass the same flag to `agentforge scaffold enhance` to upgrade an
existing project:
```bash
agentforge scaffold enhance . --agent-gateway
```
...
- `/agentforge-workflow` — Development workflow, coding guidelines, and operational rules
- `/agentforge-adk-code` — ADK Python API quick reference for writing agent code
- `/agentforge-eval` — Evaluation methodology, dataset schema, and the eval-fix loop
- `/agentforge-scaffold` — Project creation and enhancement with `agentforge scaffold create` / `scaffold enhance`
- `/agentforge-observability` — Cloud Trace, logging, BigQuery Analytics, and third-party integrations
