...
description: >
  ...
  Covers `agentforge scaffold create`, `scaffold enhance`, and `scaffold upgrade` commands,
...
Use the `agentforge` CLI to create new agent projects or enhance existing ones with deployment, CI/CD, and infrastructure scaffolding.

---

## Prerequisite: Clarify Requirements (MANDATORY for new projects)

**Before scaffolding a new project, load `/agentforge-workflow` and complete Phase 0** — clarify the user's requirements before running any `scaffold create` command. Ask what the agent should do, what tools/APIs it needs, and whether they want a prototype or full deployment.
...
| Choice | CLI flag |
|--------|----------|
| Retrieval/RAG, sandboxed execution, cross-session memory, OAuth consent, guardrails, scheduled runs | **No flag** — these come from clone-and-study recipes. **ADK:** see the topic index in `/agentforge-adk-code` → `references/samples.md`; on other frameworks, see the sample index the framework template ships |
...
> **Removed flags.** `--datastore`, the `agentic_rag` template, and `agentforge infra datastore` /
> `agentforge data-ingestion` no longer exist. If you reach for one, you want a recipe instead.
...
```bash
agentforge scaffold create <project-name> \
  --agent <template> \
...
agentforge scaffold enhance . --cicd-runner <runner>
```
...
Upgrade an existing project to a newer agentforge version, intelligently applying updates while preserving your customizations:

```bash
agentforge scaffold upgrade                # Upgrade current directory
agentforge scaffold upgrade <project-path> # Upgrade specific project
agentforge scaffold upgrade --dry-run      # Preview changes without applying
agentforge scaffold upgrade --auto-approve  # Auto-apply non-conflicting changes
```
...
agentforge scaffold enhance . --deployment-target agent_runtime

# Add CI/CD pipeline (ask: GitHub Actions or Cloud Build?)
agentforge scaffold enhance . --cicd-runner github_actions
```
...
> template is `extensions/langchain/template/` in the agentforge repo; see
> `/agentforge-workflow` → `references/extension.md` to publish your own. Capabilities
...
> `/agentforge-adk-code` → `references/samples.md`.
...
agentforge scaffold create my-agent --agent adk --prototype
...
After scaffolding, immediately load `/agentforge-workflow` — it contains the development workflow, coding guidelines, and operational rules you must follow when implementing the agent.
...
**Verifying your agent works:** Use `agentforge run "test prompt"` for quick smoke tests, then `agentforge eval run` for systematic validation. Do NOT write pytest tests that assert on LLM response content, that belongs in eval.
...
```bash
agentforge scaffold create ref-project --output-dir /tmp \
  --agent adk \
...
- **NEVER skip requirements clarification** — load `/agentforge-workflow` Phase 0 and clarify the user's intent before running `scaffold create`
...
Actions:
1. Create temp project: `agentforge scaffold create ref --output-dir /tmp --agent adk --deployment-target cloud_run`
...
2. `agentforge scaffold create my-a2a-agent --agent adk --deployment-target cloud_run --prototype`
...
See `/agentforge-workflow` → **Setup** section.

---

## Related Skills

- `/agentforge-workflow` — Development workflow, coding guidelines, and the build-evaluate-deploy lifecycle
- `/agentforge-adk-code` — ADK Python API quick reference for writing agent code (ADK projects)
