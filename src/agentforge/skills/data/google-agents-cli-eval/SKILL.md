...
description: >
  ...
  Applies to any agentforge project, whatever framework the agent is written in.
...
> **Scaffolded project?** If you used `/agentforge-scaffold`, you already have `agentforge eval run` (chains `generate` + `grade`), `tests/eval/datasets/`, and `tests/eval/eval_config.yaml`. Start with executing `eval run` and iterate from there.
...
**Opt-in (ADK projects):** `agentforge eval dataset synthesize`: user-simulate multi-turn datasets when you lack data; its output already includes traces, so Stage 2 collapses to `agentforge eval grade` alone. See *Eval Commands* and `references/user-simulation.md`.

### 2. Run the Eval (always run)

**Default:** `agentforge eval run` runs the agent over the dataset and grades the traces, writing `results_<ts>.{json,html}` to `artifacts/grade_results/`.
...
**Opt-in:** `agentforge eval analyze`, LLM-based failure clustering; prefer when you have 10+ failing cases and want categorized failure modes. See `references/advanced-commands.md`.
...
Run `agentforge eval metric list` to see all available built-ins. For full metric definitions and rubric details, see the [Agent Platform metric docs](https://cloud.google.com/gemini-enterprise-agent-platform/optimize/evaluation/manage-metrics) and `references/metrics-guide.md`.

---

## What to fix when scores fail

After `agentforge eval run` completes, inspect the latest `artifacts/grade_results/results_<timestamp>.json` (or open the `.html` file) for per-case scores and judge rationales, the input to every fix decision below.
...
After applying a fix, rerun `agentforge eval run` and use `agentforge eval compare <prev_results>.json <new_results>.json` to confirm the fix improved the target metric without regressing others.

---

## Eval Commands

`agentforge eval <subcommand> --help` is the authoritative flag list; the examples below are the common invocations.
...
agentforge eval run --dataset tests/eval/datasets/custom.json --metrics final_response_quality,safety --output ./out/
```
...
agentforge eval generate

# Advanced — custom dataset and output dir
agentforge eval generate --dataset tests/eval/datasets/custom.json -o ./custom_traces/

# Against a deployed agent (or one you started manually)
agentforge eval generate --url https://my-agent.run.app --app-name app
```
...
```bash
agentforge eval compare baseline.json candidate.json
```
...
`agentforge eval run --config <path>` (and `eval grade --config <path>`) accepts a single configuration file in either **YAML** (`.yaml` / `.yml`) or **JSON** (`.json`). The file declares two parts:
...
**Agent trace field model.** For datasets produced by `agentforge eval generate` (or `eval dataset synthesize`), each eval case exposes three standard fields to a metric:
...
- **Before deploy:** Rerun `agentforge eval run` and show the scores for every case, not just the one you fixed. `eval run` exits 0 whatever the scores are, so the numbers you paste are the gate, not the exit code.

---

## Related Skills

- `/agentforge-workflow` — Development workflow and the spec-driven build-evaluate-deploy lifecycle
- `/agentforge-adk-code` — ADK Python API quick reference for writing agent code (ADK projects only)
- `/agentforge-scaffold` — Project creation and enhancement with `agentforge scaffold create` / `scaffold enhance`
- `/agentforge-deploy` — Deployment targets, CI/CD pipelines, and production workflows
- `/agentforge-observability` — Cloud Trace, logging, and monitoring for debugging agent behavior
