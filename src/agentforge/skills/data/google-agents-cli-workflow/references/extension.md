...
- **Re-invoke the built-in safely.** An override runs with `AGENTS_CLI_DISABLE_OVERRIDES=1`, so calling `agentforge deploy` inside your wrapper hits the built-in (no infinite recursion).
