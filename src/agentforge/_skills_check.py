...
# Stamp file under the well-known agents directory. The basename
# ``.agentforge_skills_check`` carries the new project name; the parent
# ``~/.agents/`` stays as-is because that path is the industry convention
# used by Claude and other agent tools, not a project-owned location.
_SKILLS_CHECK_STAMP = Path.home() / ".agents" / ".agentforge_skills_check"

# Recognised skill-name prefixes. The new ``agentforge-*`` is the user-facing
# brand; ``google-agents-cli-*`` is retained so previously installed skills
# keep being detected (renaming an installed directory would orphan the
# user's `.agents/skills/` store).
_SKILL_NAME_PREFIXES: tuple[str, ...] = ("agentforge-", "google-agents-cli-")


def _is_agents_skill(name: str) -> bool:
    """True if ``name`` is one of the recognised AgentForge skills."""
    return any(name.startswith(prefix) for prefix in _SKILL_NAME_PREFIXES)
...
def _find_installed_skills() -> dict[str, str]:
    """Return ``{skill_name: version}`` for every installed AgentForge skill.

    Fast path (~8 ms): scan ``~/.agents/skills/{agentforge-,google-agents-cli-}*``
    ...
    if skills_dir.is_dir():
        for skill_dir in sorted(skills_dir.iterdir()):
            if not _is_agents_skill(skill_dir.name):
                continue
...
def get_installed_skills() -> list[dict] | None:
    """Return installed agentforge skills as a list of dicts.
    ...
    try:
        ...
        return [s for s in skills if _is_agents_skill(s.get("name", ""))]
    except Exception as e:
...
def check_skills_version() -> None:
    ...
    Scans all installed ``agentforge-*`` and legacy ``google-agents-cli-*``
    skills, compares each ``metadata.version`` with the running
    ``__version__``, and lists the mismatched ones.  Never blocks execution.
