...
"""agentforge run command — run agent with a single prompt."""
...
from agentforge._adk_client import create_session, run_sse
from agentforge._project import (
    chdir_project_root,
    read_project_config,
    require_agent_directory,
)
from agentforge._remote import (
...
from agentforge.run._local_server import (
    api_base_path,
    ensure_server,
    stop_server,
)
from agentforge.run._multimodal import (
    build_a2a_parts,
    build_adk_sse_parts,
    build_agent_runtime_message,
)

_ARTIFACTS_DIR = Path(".agentforge") / "artifacts"
# Legacy artifacts dir used by `run --save-artifacts` under the pre-rename
# CLI. Kept as a separate constant so a one-shot migration in this command
# can rename it on first use.
_ARTIFACTS_DIR_LEGACY = Path(".google-agents-cli") / "artifacts"
...
def cmd_run(
    ...
    *,
    ...
    saved under '.agentforge/artifacts/' in the project root and
...
def _print_session_id(session_id: str | None) -> None:
    ...
    click.secho(
        f'  Resume with: agentforge run "<message>" --session-id {session_id}',
        dim=True,
    )
...
def _write_artifact(data: bytes, mime_type: str | None) -> str:
    ...
    # One-shot migration: if the new artifacts dir is empty (or missing) and
    # the legacy dir holds prior runs, fold the legacy dir into the new
    # one so a user who previously ran `agentforge run --save-artifacts`
    # keeps access to those files.
    legacy = Path.cwd() / _ARTIFACTS_DIR_LEGACY
    if legacy.exists() and not any(artifacts_dir.iterdir()) if artifacts_dir.exists() else True:
        legacy.rename(artifacts_dir)
...
def _query_adk_sse(
    ...
    *,
    ...
    Thin CLI wrapper around :mod:`agentforge._adk_client`: creates a
    ...
    async with httpx.AsyncClient(headers=req_headers, timeout=120) as http_client:
        ...
        try:
            ...
            raise click.ClickException(
                ...
                "'agentforge scaffold upgrade', or try --mode adk."
            ) from exc
