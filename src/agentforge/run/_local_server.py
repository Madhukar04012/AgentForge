...
from agentforge._runner import popen_resolved_detached, redact_cmd
...
_PID_DIR = ".agentforge"
# Legacy pre-rename dir name. ``_resolve_pid_dir`` returns the new name and
# renames the legacy dir on first use so a freshly renamed CLI finds the
# PID file that an older run left behind.
_PID_DIR_LEGACY = ".google-agents-cli"
...
def ensure_server(
    ...
    *,
        ...
        if _is_server_alive(info["pid"], info["port"]):
            ...
            if existing_mode != use_in_memory_session:
                ...
                raise click.ClickException(
                    ...
                    "  Run 'agentforge run --stop-server' first, "
                    "then retry."
                )
            ...
            if _is_idle(info, idle_timeout):
                ...
                if trace_to_cloud and not info.get("trace_to_cloud"):
                    click.secho(
                        ...
                        "  Run 'agentforge run --stop-server' first to "
    ...
    click.secho("  Stop with: agentforge run --stop-server", dim=True)
...
def _start_server(
    *,
    ...
    # One-shot migration from the pre-rename artifact dir: if the new dir
    # is empty but the legacy dir holds a PID file / log, rename the legacy
    # dir into place so a freshly renamed CLI finds the server the user
    # started under the old name.
    if not any(adk_dir.iterdir()) if adk_dir.exists() else True:
        legacy_dir = project_root / _PID_DIR_LEGACY
        if legacy_dir.exists():
            legacy_dir.rename(adk_dir)
