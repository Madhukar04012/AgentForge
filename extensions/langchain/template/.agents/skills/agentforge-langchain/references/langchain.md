# LangChain with agentforge

This project was scaffolded from the LangChain template and carries
`agentforge-extension.yaml` at its root, which overrides the framework-coupled
...
```bash
agentforge create my-lc-agent --agent google/agentforge/extensions/langchain/template@v1.5.0 -d cloud_run --cicd-runner github_actions
```

For a project that skipped them, `agentforge scaffold enhance -d cloud_run
--cicd-runner github_actions` re-renders from the template recorded in
`agentforge-manifest.yaml` as `base_template`. It fetches that template, so it
...
- `deploy` — native `agentforge deploy`; dispatches to the project's configured deployment target.
...
```bash
agentforge create my-lc-agent --agent google/agentforge/extensions/langchain/template@v1.5.0 -d cloud_run
cd my-lc-agent && agentforge install
agentforge run "hello"                     # in-process graph invocation
...
agentforge deploy                          # native deploy (by deployment target)
```
...
| `GOOGLE_CLOUD_AGENT_ENGINE_ENABLE_TELEMETRY` | Set by `agentforge deploy` on Agent Runtime; `false` turns Cloud export off |
...
- To run the built-in instead of the override: `AGENTFORGE_DISABLE_OVERRIDES=1 agentforge <cmd>`.
