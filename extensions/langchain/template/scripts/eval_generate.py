...
"""LangChain trace generator — drop-in for `agentforge eval generate`.

Produces the same EvaluationDataset shape the built-in `agentforge eval generate`
emits, so `agentforge eval grade` consumes the output unchanged.
"""
...
def _agent_name() -> str:
    default_name = "agent"
    manifest = Path("agentforge-manifest.yaml")
...
def main() -> int:
    ...
    if args.url:
        print(
            ...
            "AGENTFORGE_DISABLE_OVERRIDES=1 agentforge eval generate --url ...",
            file=sys.stderr,
        )
        return 2
