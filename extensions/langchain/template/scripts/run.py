...
"""One-shot agent-graph runner — drop-in for `agentforge run`.
...
queries against a deployed service, bypass the override:
    ...
    AGENTFORGE_DISABLE_OVERRIDES=1 agentforge run --url https://... --mode a2a "hi"
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="agentforge run", description=__doc__)
    ...
    if args.url or args.mode:
        print(
            ...
            'AGENTFORGE_DISABLE_OVERRIDES=1 agentforge run --url ... --mode a2a "hi"',
            file=sys.stderr,
        )
        return 2
