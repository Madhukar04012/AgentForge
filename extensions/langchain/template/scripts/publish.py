...
def _deployment_target() -> str:
    manifest = pathlib.Path("agentforge-manifest.yaml")
...
def main() -> int:
    if _deployment_target() == "agent_runtime":
        print(
            "agentforge publish gemini-enterprise is not supported by the langchain "
    ...
    env = {**os.environ, "AGENTFORGE_DISABLE_OVERRIDES": "1"}
    return subprocess.run(
        ["agentforge", "publish", "gemini-enterprise", *sys.argv[1:]], env=env
    ).returncode
