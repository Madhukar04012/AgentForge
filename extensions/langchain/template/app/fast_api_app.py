...
contract used by agentforge's ADK A2A agents, so the deploy entrypoint
(``uvicorn app.fast_api_app:app``) and ``agentforge run --mode a2a`` keep
working unchanged.

For the richest *local* LangGraph experience (graph inspection, streaming,
threads), ``agentforge playground`` runs ``langgraph dev``, which also serves
