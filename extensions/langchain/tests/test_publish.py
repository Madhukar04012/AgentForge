...
def test_only_agent_runtime_is_refused(monkeypatch, tmp_path, capsys, target, refused):
    (tmp_path / "agentforge-manifest.yaml").write_text(
    ...
    if refused:
        ...
        assert code == 0
        assert calls[0]["argv"][:3] == ["agentforge", "publish", "gemini-enterprise"]
        assert calls[0]["argv"][3:] == ["--registration-type", "a2a"]
        assert calls[0]["env"]["AGENTFORGE_DISABLE_OVERRIDES"] == "1"
