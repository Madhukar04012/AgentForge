...
from agentforge.extension._spec import parse_extension_spec
...
def _spec():
    data = yaml.safe_load((_TEMPLATE_DIR / "agentforge-extension.yaml").read_text())
...
class TestManifestMatchesPublishedSchema:
    def test_first_party_manifest_validates(self):
        ...
        from agentforge.extension._schema import build_json_schema

        data = yaml.safe_load(
            (_TEMPLATE_DIR / "agentforge-extension.yaml").read_text(encoding="utf-8")
        )
...
def project(tmp_path_factory):
    ...
    from agentforge.scaffold.commands.create import create
...
class TestScaffoldedProject:
    ...
    def test_it_carries_its_own_overrides(self, project):
        ...
        assert (project / "agentforge-extension.yaml").is_file()
        # .template is the CLI's to read, not the project's to carry.
        assert not (project / ".template").exists()
        assert (project / ".agents/skills/agentforge-langchain/SKILL.md").is_file()
...
class TestShippedSkill:
    def test_it_is_not_installable_from_this_repo(self):
        ...
        """
        assert not (_REPO_ROOT / "skills" / "agentforge-langchain").exists()
