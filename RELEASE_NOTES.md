...
### Changed
- **Project rename: `google-agents-cli` → `AgentForge`.** The Python
  package is now `agentforge` (was `google.agents.cli`); the primary
  console script is `agentforge`. The previous `agents-cli` console
  script is retained as a back-compat alias for one release cycle so
  existing shell aliases, CI scripts, and muscle memory keep working.
  - The PyPI distribution is now `agentforge`; installs under the
    legacy `google-agents-cli` distribution name continue to be
    recognised, and `version()` falls back to the legacy name so a
    pre-rename install still reports a useful version.
  - Environment variables renamed: `AGENTFORGE_DISABLE_OVERRIDES`,
    `AGENTFORGE_EXTENSION_DIR`, `AGENTFORGE_EXPERIMENTS`. The legacy
    `AGENTS_CLI_*` names are still read as a fallback (new name wins
    if both are set).
  - On-disk config paths renamed: `~/.config/agentforge/`,
    `~/.cache/agentforge/git/`, `%APPDATA%\agentforge\` (with a
    one-time `DeprecationWarning` if the legacy `agents-cli` path is
    still present).
  - User-project files: `agentforge-manifest.yaml` (was
    `agents-cli-manifest.yaml`); `[tool.agentforge]` pyproject section
    (was `[tool.agents-cli]`); `.agentforge/` artifact dir (was
    `.google-agents-cli/`); `.agentforge-scripts/` (was
    `.agents-cli-scripts/`). The legacy filenames/dirs are still read;
    the first write to a project folds the legacy paths forward.
  - Skill directories renamed: `skills/agentforge-*/` (was
    `skills/google-agents-cli-*/`); the old skill dirs are still
    loaded until users run `npx skills update`.
  - The extension schema (`agents-cli-extension-v1alpha1`) and the
    `agentforge-extension.yaml` filename accept both the new
    `requires.agentforge` and the legacy `requires.agents_cli` fields;
    new wins if both are set.
  - The skills loader, the Pydantic extension spec, the manifest
    filename, the `tool.agents-cli` pyproject migration, the
    `.google-agents-cli/` artifact rename, the LangChain extension
    manifest, the cookiecutter template variables, and the
    `update_check` stamp file have all been rewritten to the new
    identity; tests in `tests/test_agentforge_compat.py` pin the
    compat layer.
- The web-platform backend's PyPI distribution was renamed from
  `agentforge-api` to `agentforge` so the monorepo ships a single
  `uv sync`-installable distribution covering both the CLI and the
  web backend. The CLI's entry point is `agentforge.main:main`; the
  back-compat alias `agents-cli` continues to work.

### Notes
- Apache-2.0 license headers and `Copyright 2026 Google LLC`
  attribution lines are preserved verbatim — these are legal
  attribution, not product identity.
- The upstream repository URL (`https://github.com/google/agents-cli`)
  is preserved as-is; the user has not moved the repository, and the
  migration does not invent fake `github.com/agentforge/...` URLs.
- Google technology surface (Cloud Run, Vertex AI, BigQuery, ADK,
  `google-genai`, `google-cloud-*` packages) is preserved.

### Added
- **Web platform foundation (Phase 1).** A new `agentforge_api` Python
  package provides a FastAPI backend alongside the existing
  `google-agents-cli` CLI. A new `web/` directory hosts the Vite + React
  + TypeScript SPA. Docker images + a `docker-compose.yml` bring the
  stack up locally. Architecture docs are committed under
  `docs/architecture/`.
  - `GET  /api/v1/health` — liveness probe.
  - `POST /api/v1/auth/login` / `POST /api/v1/auth/logout` —
    placeholder email + password login.
  - `GET  /api/v1/me` — current user.
  - `POST /api/v1/demo/session` and `GET /api/v1/demo/project` —
    recruiter demo entry (placeholder content; full seed lands in
    Phase 14).
- `pyproject.toml` at the repo root declaring the `agentforge_api`
  package and a `uv` workspace. The CLI source trees
  (`src/google/agents/cli/`, `src/agentforge/`) are unchanged.

### Notes
- The CLI is fully backward-compatible. `agents-cli --version` and
  `agentforge --version` continue to work and report the same
  version. No existing tests are touched.
...
**AgentForge is now 1.0 — Generally Available.** This is our first GA release: a stable, production-ready CLI for scaffolding, evaluating, and deploying ADK agents on Google Cloud.
...
- Visual Explainer page for AgentForge lifecycle at https://google.github.io/agents-cli/
